...
public class Menu {
	...
	protected void generarConfirmacion(String pregunta, String dni, String nombre, String apellidos, String nick,
			String pass) {
	...
	protected void mostrarConsultas(Usuario user, Cliente cliente, ArrayList<Consulta> consultas) {
		...
		while (it.hasNext()) {
			...
			if (c.getCliente().equals(cliente)) {
				...
				fila(consultas.indexOf(c) + " - | " + titulo + " | " + (c.isNuevoMensajeTecnico() ? "Nuevo" : "Leido")
						+ " | " + c.getnMensajes() + " Mensajes");
			}
		}
		if(nConsultas <= 0) {
			fila("No tienes consultas abiertas.");
		}
		interFila();
		fila("x - Salir");
		ultimaFila();
	...
	protected void mostrarTodasLasConsultas(ArrayList<Consulta> consultas) {
		Iterator<Consulta> it = consultas.iterator();
		String titulo;
		int nConsultas = 0;
		ultimaFila();
		cabezera("TODAS LAS CONSULTAS");
		while (it.hasNext()) {
			nConsultas++;
			Consulta c = it.next();
			titulo = espacios(c.getTitulo(), mayorEnLista(consultas));
			fila(consultas.indexOf(c) + " - | " + titulo + " | " + (c.isNuevoMensajeCliente() ? "Nuevo" : "Leido")
					+ " | " + c.getnMensajes() + " Mensajes");

		}
		if(nConsultas <= 0) {
			fila("No hay consultas registradas.");
		}
		interFila();
		fila("x - Salir");
		ultimaFila();
	...
	protected void mostrarHistorial(ArrayList<Consulta> consultas) {
		Iterator<Consulta> it = consultas.iterator();
		String titulo;
		int nConsultas = 0;
		ultimaFila();
	...
	protected void mostrarConsultasConMensajesNuevos(ArrayList<Consulta> consultas) {
		Iterator<Consulta> it = consultas.iterator();
		String titulo;
		int nConsultas = 0;
		ultimaFila();
		cabezera("MENSAJES NUEVOS");
		while (it.hasNext()) {
			Consulta c = it.next();
			if(c.isNuevoMensajeCliente()) {
				nConsultas++;
				titulo = espacios(c.getTitulo(), mayorEnLista(consultas));
				fila(consultas.indexOf(c) + " - | " + titulo + " | " + (c.isNuevoMensajeCliente() ? "Nuevo" : "Leido")
						+ " | " + c.getnMensajes() + " Mensajes");
		...
		if(nConsultas <= 0) {
			fila("No hay consultas con mensajes nuevos.");
		}
		interFila();
		fila("x - Salir");
		ultimaFila();
	...
	protected void mostrarConsultasNuevas(ArrayList<Consulta> consultas) {
		Iterator<Consulta> it = consultas.iterator();
		String titulo;
		int nConsultas = 0;
		ultimaFila();
		cabezera("CONSULTAS NUEVAS");
		while (it.hasNext()) {
			Consulta c = it.next();
			if(c.isNueva()) {
				nConsultas++;
